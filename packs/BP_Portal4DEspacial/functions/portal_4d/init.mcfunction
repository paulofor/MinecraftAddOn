...
tellraw @s {"rawtext":[{"text":"[Portal4D] API Microsoft: o BP usa @minecraft/server 2.0.0-beta e tenta registerCustomDimension no system.beforeEvents.startup para criar portal4d:espaco_4d."}]}
tellraw @s {"rawtext":[{"text":"[Portal4D] Se a Custom Dimension API/Beta APIs nao estiverem ativas, o destino cai para a arena fallback no Overworld em 4096 96 4096."}]}
