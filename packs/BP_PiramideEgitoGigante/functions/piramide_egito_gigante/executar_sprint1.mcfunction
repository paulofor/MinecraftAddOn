...
execute if score @s peg_bloqueio matches 0 run effect @s slow_falling 20 1 true
execute if score @s peg_bloqueio matches 0 run tp @s ~ ~8 ~
execute if score @s peg_bloqueio matches 0 run tellraw @s {"rawtext":[{"text":"[Piramide][Sprint 1] Voce foi movido alguns blocos acima da fundacao com queda lenta para evitar sufocamento durante a validacao visual."}]}
