...
class Portal4DFunctionTests(unittest.TestCase):
  ...
  def test_portal_manifests_are_paired_at_0_1_39(self) -> None:
    ...
    self.assertEqual([0, 1, 39], bp["header"]["version"])
    self.assertEqual(bp["header"]["version"], rp["header"]["version"])
    self.assertTrue(all(module["version"] == [0, 1, 39] for module in bp["modules"] + rp["modules"]))
    self.assertEqual([0, 1, 39], bp["dependencies"][0]["version"])
