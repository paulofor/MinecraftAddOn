...
class BedrockCommandAllowlistTest(unittest.TestCase):
  ...
  def test_plan_build_location_requires_approval_before_command_generation(self) -> None:
    suggestion = {
      "recommended_center": {"x": -194, "y": 69, "z": 111},
      "affected_area": {"x": [-258, -130], "y": [47, 114], "z": [47, 175]},
      "confidence": "medium",
      "reasons": ["teste"],
      "warnings": [],
    }
    with mock.patch.object(server, "_suggest_arena_location", return_value=suggestion), \
      mock.patch.object(server, "_sample_build_blocks", return_value={"samples": [], "errors": [], "risky_blocks": []}):
      plan = server._plan_build_location(approval_confirmed=False)

    self.assertTrue(plan["approval_required"])
    self.assertIsNone(plan["command_after_approval"])
    self.assertEqual(plan["recommended_center"], {"x": -194, "y": 69, "z": 111})
    self.assertEqual(plan["size"], {"x": 129, "y": 68, "z": 129})

  def test_plan_build_location_generates_command_after_clean_approval(self) -> None:
    suggestion = {
      "recommended_center": {"x": -194, "y": 69, "z": 111},
      "affected_area": {"x": [-258, -130], "y": [47, 114], "z": [47, 175]},
      "confidence": "medium",
  ...
  def test_execute_planned_build_dry_run_after_clean_approval(self) -> None:
    command = "execute positioned -194 69 111 run function piramide_egito_gigante/montar_completa"
    plan = {
    ...
    with mock.patch.object(server, "_plan_build_location", return_value=plan), \
      mock.patch.object(server, "_append_bedrock_command_audit"):
      result = server._execute_planned_build(approval_confirmed=True, execute=False, executor="unit-test")

    self.assertEqual(result["status"], "dry_run_ready")
    self.assertEqual(result["execution"]["would_send"], command)
    self.assertEqual(result["execution"]["executor"], "unit-test")
    self.assertEqual(result["execution"]["expected_confirmation_token"], "EXECUTAR_piramide_egito_gigante_-194_69_111")

  def test_execute_planned_build_blocks_when_confirmation_token_is_missing(self) -> None:
    command = "execute positioned -194 69 111 run function piramide_egito_gigante/montar_completa"
    plan = {
      "approval_required": False,
      "recommended_center": {"x": -194, "y": 69, "z": 111},
    ...
    self.assertEqual(result["status"], "blocked_by_confirmation")
    self.assertEqual(result["execution"]["expected_confirmation_token"], "EXECUTAR_piramide_egito_gigante_-194_69_111")

  def test_execute_planned_build_sends_after_clean_approval(self) -> None:
    command = "execute positioned -194 69 111 run function piramide_egito_gigante/montar_completa"
    plan = {
      "approval_required": False,
      "recommended_center": {"x": -194, "y": 69, "z": 111},
    ...
    with mock.patch.object(server, "_plan_build_location", return_value=plan), \
      mock.patch.object(server, "_run_bedrock_command", return_value={"status": "sent", "command": command}) as run_command, \
      mock.patch.object(server, "_read_optional_tail", return_value={"available": True, "content": "ok"}):
      result = server._execute_planned_build(
        approval_confirmed=True,
        execute=True,
        executor="unit-test",
